...
<% string property = ViewData.ModelMetadata.PropertyName; %>
<script>
/// jQuery plugin to add support for SwfUpload
/// (c) 2008 Steven Sanderson

(function($) {
    $.fn.<%= property %>makeAsyncUploader = function(options) {
            ...
            var defaults = {
                ...
                upload_success_handler: function(file, response) {
                    ...
                    $("span[id$=_completedMessage]", container).html("Uploaded <b>{0}</b> ({1} KB)"
                     ...
                     $("#<%=property.Replace("_guid", "") %>Image").attr("src", "/MediaFile/Image/" + response + "?H=100&W=200");
                },
...
<% 
   ...
   { %>
    <img id="<%=property.Replace("_guid", "") %>Image" src="<%= ResolveUrl("~/")%>MediaFile/Image/<%= ViewData.Model %>" alt="<%= ViewData.ModelMetadata.Description %>" />
    <div class="button flash">
    <input type="file" name="<%=property.Replace("_guid", "") %>" id="<%=property.Replace("_guid", "") %>" style="width:200px;"  />
    </div>
<% }else{ %>

    <img id="<%=property.Replace("_guid", "") %>Image" src="<%= ResolveUrl("~/")%>MediaFile/NoPhoto?H=100&W=200" alt="" />
    <div class="button flash">
    <input type="file" name="<%=property.Replace("_guid", "") %>" id="<%=property.Replace("_guid", "") %>" style="width:200px;"  />
    </div>
<%} %>
<script>
        $(function() {
             $("#<%=property.Replace("_guid", "") %>").<%= property %>makeAsyncUploader({
                upload_url: "/MediaFile/UploadPicture",
                flash_url: '/Scripts/swfupload.swf',
                button_image_url: '/Content/blankButton.png',
                disableDuringUpload: 'button[type="submit"]',
                button_text: '<%= GlobalMessages.SELECT_FILE %>',
                button_width: 200
