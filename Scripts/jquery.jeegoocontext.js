...
(function ($) {
    ...
    $.fn.jeegoocontext = function (id, options) {
        ...
        return this[_menus[id].livequery ? 'livequery' : 'bind'](eventType, function (e) {
            ...
            log("setting");
            $menu.css({  // I would add this line 'left': startLeft + 'px', 'top': startTop + 'px' }).fadeIn(_menus[id].fadeIn); 
                'left': startLeft + 'px',
                'top': startTop + 'px'
            ...
            if (_menus[id].onShowEnd) {
                if (_menus[id].onShowEnd.apply($menu, [e, _menus[id].context, startLeft, startTop]) == false) {
                    return false;
                }
            }
